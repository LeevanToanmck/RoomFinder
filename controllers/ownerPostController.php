<?php
require_once "./../models/PostModel.php";
require_once "./../models/ImageModel.php";
require_once "./../models/PostTypeModel.php";

class ownerPostController
{
    private $postModel;
    private $imageModel;
    private $typeModel;

    public function __construct()
    {
        $this->postModel = new PostModel();
        $this->imageModel = new ImageModel();
        $this->typeModel = new PostTypeModel();
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
    }

    private function ensureOwner()
    {
        if (!isset($_SESSION['user_id'])) {
            header("Location: ./client.php?controller=auth&action=login");
            exit();
        }
        if (!isset($_SESSION['role']) || ($_SESSION['role'] !== 'owner' && $_SESSION['role'] !== 'admin')) {
            echo "<script>alert('Chỉ tài khoản người cho thuê (owner) mới được đăng bài.'); window.location.href='./client.php';</script>";
            exit();
        }
    }

    private function normalizeMultiFiles($files)
    {
        $out = [];
        if (!isset($files['name']) || !is_array($files['name'])) {
            return $out;
        }
        $count = count($files['name']);
        for ($i = 0; $i < $count; $i++) {
            $out[] = [
                'name' => $files['name'][$i],
                'tmp_name' => $files['tmp_name'][$i] ?? null,
                'error' => $files['error'][$i] ?? UPLOAD_ERR_NO_FILE,
            ];
        }
        return $out;
    }

    private function storeUploadedImagesOrFail($post_id, $minCount = 0)
    {
        $files = isset($_FILES['images']) ? $this->normalizeMultiFiles($_FILES['images']) : [];
        $valid = array_values(array_filter($files, fn($f) => ($f['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK));

        if ($minCount > 0 && count($valid) < $minCount) {
            echo "<script>alert('Vui lòng chọn tối thiểu {$minCount} hình ảnh cho bài post!'); window.history.back();</script>";
            exit();
        }
        if (empty($valid)) {
            return;
        }

        foreach ($valid as $f) {
            $ext = strtolower(pathinfo($f['name'], PATHINFO_EXTENSION));
            if (!in_array($ext, ['jpg', 'jpeg', 'png', 'webp', 'gif'], true)) {
                echo "<script>alert('File ảnh không hợp lệ: {$f['name']}'); window.history.back();</script>";
                exit();
            }

            $safeName = uniqid('post_', true) . '.' . $ext;
            $destRel = 'image/' . $safeName;
            $destAbs = './../public/' . $destRel;

            if (!move_uploaded_file($f['tmp_name'], $destAbs)) {
                echo "<script>alert('Upload ảnh thất bại!'); window.history.back();</script>";
                exit();
            }

            $this->imageModel->add($post_id, $destRel);
        }
    }

    public function index()
    {
        $this->ensureOwner();
        $myId = (int)$_SESSION['user_id'];

        // simple: reuse getAll then filter in PHP (small data). If large, add model method later.
        $all = $this->postModel->getAll();
        $posts = array_values(array_filter($all, fn($p) => (int)$p['user_id'] === $myId));

        include "./../views/client/owner/posts/index.php";
    }

    public function add()
    {
        $this->ensureOwner();
        $types = $this->typeModel->getAll();
        include "./../views/client/owner/posts/add.php";
    }

    public function store()
    {
        $this->ensureOwner();
        $user_id = (int)$_SESSION['user_id'];
        $type_id = (int)($_POST['type_id'] ?? 0);
        $title = trim($_POST['title'] ?? '');
        $description = trim($_POST['description'] ?? '');
        $price = (float)($_POST['price'] ?? 0);
        $area = (float)($_POST['area'] ?? 0);
        $address = trim($_POST['address'] ?? '');
        $city = trim($_POST['city'] ?? '');
        $district = trim($_POST['district'] ?? '');
        $status = $_POST['status'] ?? 'available';

        $this->postModel->add($user_id, $type_id, $title, $description, $price, $area, $address, $city, $district, $status);
        $post_id = $this->postModel->lastInsertId();
        $this->storeUploadedImagesOrFail($post_id, 3);

        header("Location: ./client.php?controller=ownerPost&action=index");
        exit();
    }

    public function edit()
    {
        $this->ensureOwner();
        $id = (int)($_GET['id'] ?? 0);
        $post = $this->postModel->findById($id);
        if (!$post || (int)$post['user_id'] !== (int)$_SESSION['user_id']) {
            header("Location: ./client.php?controller=ownerPost&action=index");
            exit();
        }
        $images = $this->imageModel->getByPostId($id);
        $types = $this->typeModel->getAll();
        include "./../views/client/owner/posts/edit.php";
    }

    public function update()
    {
        $this->ensureOwner();
        $id = (int)($_POST['id'] ?? 0);
        $post = $this->postModel->findById($id);
        if (!$post || (int)$post['user_id'] !== (int)$_SESSION['user_id']) {
            header("Location: ./client.php?controller=ownerPost&action=index");
            exit();
        }

        $type_id = (int)($_POST['type_id'] ?? 0);
        $title = trim($_POST['title'] ?? '');
        $description = trim($_POST['description'] ?? '');
        $price = (float)($_POST['price'] ?? 0);
        $area = (float)($_POST['area'] ?? 0);
        $address = trim($_POST['address'] ?? '');
        $city = trim($_POST['city'] ?? '');
        $district = trim($_POST['district'] ?? '');
        $status = $_POST['status'] ?? 'available';

        $this->postModel->update($id, (int)$_SESSION['user_id'], $type_id, $title, $description, $price, $area, $address, $city, $district, $status);

        $files = isset($_FILES['images']) ? $this->normalizeMultiFiles($_FILES['images']) : [];
        $hasNew = false;
        foreach ($files as $f) {
            if (($f['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
                $hasNew = true;
                break;
            }
        }
        if ($hasNew) {
            $this->imageModel->deleteByPostId($id);
            $this->storeUploadedImagesOrFail($id, 0);
        }

        header("Location: ./client.php?controller=ownerPost&action=index");
        exit();
    }

    public function delete()
    {
        $this->ensureOwner();
        $id = (int)($_GET['id'] ?? 0);
        $post = $this->postModel->findById($id);
        if ($post && (int)$post['user_id'] === (int)$_SESSION['user_id']) {
            $this->postModel->delete($id);
        }
        header("Location: ./client.php?controller=ownerPost&action=index");
        exit();
    }
}

